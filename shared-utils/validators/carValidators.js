...
const CarValidators = {
  createCarSchema: Joi.object({
    ...
    plateNumber: Joi.string()
      .trim()
      .pattern(/^(0[1-9]|[1-7][0-9]|8[01])\s?([A-Z]{1,3})\s?(\d{2,4})$/i)
      .required()
      .messages({
        "string.empty": "Plaka alanı boş olamaz.",
        "any.required": "Plaka alanı zorunludur.",
        "string.pattern.base":
          "Geçerli bir plaka formatı giriniz. Örnek: 34ABC123 veya 06 AB 4567",
      }),
  ...
  updateCarSchema: Joi.object({
    ...
    features: Joi.array().items(Joi.string().trim().min(1).max(100)).optional(),
